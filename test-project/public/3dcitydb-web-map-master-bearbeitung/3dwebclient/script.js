...
var extent = new Cesium.Rectangle.fromDegrees(11.5058379, 48.1557493, 11.5431508, 48.1426405);
cesiumCamera.setView({destination: extent});
...
async function initClient() {
    ...
    // Define the home extent in Munich
    /*var homeExtent = Cesium.Rectangle.fromDegrees(11.5058379, 48.1557493, 11.5431508, 48.1426405);
    ...
    cesiumHomeButton.onclick = function () {
        cesiumViewer.camera.setView({ destination: homeExtent });*/

    // Define the home extent in Munich
    var homeExtent = Cesium.Rectangle.fromDegrees(11.5058379, 48.1557493, 11.5431508, 48.1426405);

    // Override Cesium’s home button default action
    cesiumViewer.homeButton.viewModel.command.beforeExecute.addEventListener(function (e) {
    e.cancel = true; // Prevent default behavior (zooming out to the whole globe)
    cesiumViewer.camera.setView({ destination: homeExtent }); // Set view to Munich extent
    });
...
//adding PPA citymodel with button
/*let kmlLayer = null;
function addKmlLayer() {
    var kmlUrl = "../examples/PPA_costum_web_map/data/city_model/test_geometry_MasterJSON.json"; // Replace with your actual KML file name
    ...
    var options = {
        ...
        maxCountOfVisibleTiles: 1000,
        maximumScreenSpaceError: 8
    };

    kmlLayer = new CitydbKmlLayer(options);
    loadLayerGroup([kmlLayer]); // Load the layer using the existing function
}

document.getElementById("addKmlLayerButton").addEventListener("click", addKmlLayer);
*/
...
function addKmlLayer() {
    ...
    var options = {
        ...
        maxCountOfVisibleTiles: 1000,
        maximumScreenSpaceError: 8,
        thematicDataUrl: thematicDataUrl,  // Add thematic data URL to options
        tableType: "Horizontal"  // Set the tableType for Embedded data
    };

    // Create the KML Layer
    kmlLayer = new CitydbKmlLayer(options);

    // Set thematic data source and table type
    addLayerViewModel.thematicDataSource = "GoogleSheets";  // Hardcode as Embedded
    addLayerViewModel.tableType = "Horizontal";  // Horizontal table for embedded data
    addLayerViewModel.thematicDataUrl = thematicDataUrl;  // Provide the URL for the thematic data

    // Trigger the thematic data source change (this will call initDataSourceController)
    thematicDataSourceAndTableTypeDropdownOnchange();

    // After the thematic data is set up, load the KML layer
    loadLayerGroup([kmlLayer]);
}
...
//remove PPA citymodel with button
function removeKmlLayer() {

    const layerId = kmlLayer.layerId;
    document.getElementById(layerId).remove();
    webMap.removeLayer(layerId);
...
function addWMS() {
    ...
    update(function () {
        const baseLayerPickerViewModel = cesiumViewer.baseLayerPicker.viewModel;
        const wmsUrl = "http://ppavgeos.eastus.azurecontainer.io:8080/PPA/wms?service=WMS";
        ...
        providerViewModel = new Cesium.ProviderViewModel({
            name: "wms_layer",
            iconUrl: iconUrl,
            tooltip: "This WMS Layer shows the Landusage",
            creationFunction: function () {
                return new Cesium.WebMapServiceImageryProvider({
                    url: new Cesium.Resource({
                        url: wmsUrl,
...
// Event listener for the button click to add the WMS layer
document.getElementById("addWMSLayerButton").addEventListener("click", addWMS);
...
document.getElementById("removeWMSLayerButton").addEventListener("click", removeWMS);


//add Environmental Analysis
function addNoiseMap() {
    function update(callback) {
        removeImageryProvider(false);
        callback();
    }
    ...
    update(function () {
        ...
        providerViewModel = new Cesium.ProviderViewModel({
            name: "wms_layer",
            iconUrl: iconUrl,
            tooltip: "This WMS Layer shows the Noise Map.",
            creationFunction: function () {
                return new Cesium.WebMapServiceImageryProvider({
                    url: new Cesium.Resource({
                        url: wmsUrl,
...
// Event listener for the button click to add the WMS layer
document.getElementById("addNoiseMapButton").addEventListener("click", addNoiseMap);
...
document.getElementById("removeNoiseMapLayerButton").addEventListener("click", removeNoiseMap);

// Add WMS Layer without removing existing basemap
/*function addWMS() {
    const baseLayerPickerViewModel = cesiumViewer.baseLayerPicker.viewModel;
    const wmsUrl = "http://20.75.255.74:8080/PPA/wms";
    ...
    providerViewModel = new Cesium.ProviderViewModel({
        name: "wms_layer",
        iconUrl: iconUrl,
        tooltip: "This WMS Layer shows something",
        creationFunction: function () {
            return new Cesium.WebMapServiceImageryProvider({
                url: new Cesium.Resource({
                    url: wmsUrl,
    ...
    //baseLayerPickerViewModel.selectedImagery = providerViewModel;
    imageryInserted = true;
    // Add the WMS layer to the imagery layers collection without removing the base layer
    //cesiumViewer.imageryLayers.addImageryProvider(providerViewModel);
    //imageryInserted = true;
}
...
function addNewDesignLayer() {
    ...
    TilesLayerNewDesign = new Cesium3DTilesDataLayer(options);
    loadLayerGroup([TilesLayerNewDesign]); // Load the layer using the existing function
}

document.getElementById("addNewDesignLayerButton").addEventListener("click", addNewDesignLayer);

//remove new design with button
function removeNewDesignLayer() {

    const layerId = TilesLayerNewDesign.layerId;
    document.getElementById(layerId).remove();
    webMap.removeLayer(layerId);

    TilesLayerNewDesign = null;
}

document.getElementById("removeNewDesignLayerButton").addEventListener("click", removeNewDesignLayer);
...
function addOldDesignLayer() {
    ...
    TilesLayerOldDesign = new Cesium3DTilesDataLayer(options);
    loadLayerGroup([TilesLayerOldDesign]); // Load the layer using the existing function
}

document.getElementById("addOldDesignLayerButton").addEventListener("click", addOldDesignLayer);

//remove new design with button
function removeOldDesignLayer() {

    const layerId = TilesLayerOldDesign.layerId;
    document.getElementById(layerId).remove();
    webMap.removeLayer(layerId);

    TilesLayerOldDesign = null;
}

document.getElementById("removeOldDesignLayerButton").addEventListener("click", removeOldDesignLayer);
...
function addPointCloudLayer(){
    ...
    TilesLayerPointCloud = new Cesium3DTilesDataLayer(options);
    loadLayerGroup([TilesLayerPointCloud]); // Load the layer using the existing function
}
document.getElementById("addPointCloudLayerButton").addEventListener("click", addPointCloudLayer);

//remove point cloud
function removePointCloudLayer() {

    const layerId = TilesLayerPointCloud.layerId;
    document.getElementById(layerId).remove();
    webMap.removeLayer(layerId);

    TilesLayerPointCloud = null;
}

document.getElementById("removePointCloudLayerButton").addEventListener("click", removePointCloudLayer);
